...
/*
	»нициализаци€ собственного указател€
*/
...
{	
	...
	secPlaying = 0;						//врем€ играющей песни
	id_timer = 0;						//ќпределитель таймера
	idTimerBySpectr = 1;
...
/*
	ѕроверка версии BASS и »нициализаци€ устройства 
*/
INT Application::CheckedInitBASS()
{
	if (HIWORD(BASS_GetVersion()) != BASSVERSION)
	{
		MessageBox(NULL, TEXT("ќшибка версии BASS."), NULL, 0);
		return 1;
	}
	if (!BASS_Init(-1, 44100, BASS_DEVICE_3D, 0, NULL))
	{
		MessageBox(NULL, TEXT("Ќе удалось инициализировать BASS."), NULL, 0);
		return 1;
	}
}
/*
	ѕроверка на открытие копий приложени€
*/
...
{
	if (hwndCtl == hTBSoundVolume)								//настройка регул€тора громкости
	{
	...
	{
		INT nPos = 0;				//нова€ позици€
		nPos = SendMessage(hTBPlayingSong, TBM_GETPOS, 0, 0);				//получение новой позиции
		secPlaying = nPos;													//секунды проигрывани€ = текущей позиции
		QWORD bytePos = BASS_ChannelSeconds2Bytes(hStream, nPos);			//перевод позиции в байты (дл€ перемотки)
...
VOID Application::prev()
{
	...
	INT prev = 0;							//след играюща€ песн€
	if (allSongs > 0)
	{
		//ѕоиск текущей играемой песни
		...
		//≈сли при нажатии на PrevtSong достигнут начало списка, перевести указатель на конец списка
		if (prev - 1 < 0)
			prev = allSongs - 1;
		else
			prev--;
...
VOID Application::next()
{
	...
	if (allSongs > 0)
	{
		...
		//≈сли при нажатии на NextSong достигнут конец списка, перевести указатель на начало списка
		if (next + 1 > allSongs)
			next = 0;
		stop(hStream);				//остановка потока
